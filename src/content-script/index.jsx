  ...
  Browser.runtime.onMessage.addListener(async (message) => {
    if (message.type === 'CREATE_CHAT') {
      ...
      if (data.itemId in toolsConfig) {
        const overrides = userConfig.selectionToolsOverrides
        const override = overrides && overrides[data.itemId]
        if (override) {
          prompt = override.prompt.replace('{{selection}}', data.selectionText)
        } else {
